...
``` r
### ERROR HERE ### 
library(dslabs) #change load.packeges to library
library(tidyverse)
```
...
```
    ...
    ## Installing package into 'C:/Users/jacky/AppData/Local/R/win-library/4.3'
...
movieLens %>%
  filter(!genres == "Drama") %>%
  select(title, genres, everything()) #insert everything after title and genres
```
...
### ERROR HERE ###
movieLens <- movieLens %>%
  rename(user_id = userId, #change == to =
         movie_id = movieId) #change == to =
```
...
### ERROR HERE ###
movieLens %>%
  count(year) # I changed tally(year) to count(year)
```
...
### ERROR HERE ###
movieLens %>%
  count(title, rating, sort = TRUE) #changed count(c(title, rating), sort = TRUE) to count(title, rating, sort = TRUE)
```
...
``` r
## added the group_by(title) and change mutate to summarize
```
...
starWars %>%
  group_by(species) %>%
  summarise(across(c("height", "mass"), function(x) min(x, na.rm=TRUE))) #added c() to select height and mass for the across function
```
...
fakeStarWars <- tribble(
  ~name,            ~birth_weight,  ~birth_year, ~birth_location, #added comma
...
) #added quotation mark to birth locations
```
